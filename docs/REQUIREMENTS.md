...
| **D26** | §5 blank lines | Blocks are emitted compact. The gaps in the PDF are LaTeX spacing between consecutive verbatim chunks, evidenced by the pre-game and roll-off blocks, whose lines are consecutive. All §5 output goes through helpers so the convention can be inverted in one place if the lecturer says otherwise |
