...
| **D21** | How the current loan interest rate moves | Seeds at Table 9's Stable Economy 8%. Inflation applies LK 14 multiplicatively. **Large event shifts are relative**: Economic Recession's "+15%" is ×1.15, giving 8 → 9, which reproduces §5's `Current Loan Interest : 9%`; Stock Market Boom's "−10%" is ×0.90. **The explicit ±2% adjustments are percentage points** — *Reduce Loan Interest* (LK 24) and *Interest Rate Cut* / *Interest Rate Increase* (App A) phrase themselves as direct adjustments to the rate, and a relative 2% would round to a no-op at any plausible rate, which cannot be the intended reading. Two effect kinds therefore exist, `EFF_INTEREST_MUL` and `EFF_INTEREST_ADD`; additive shifts apply first, then multiplicative. Each loan freezes its issued rate for life (LK 13). **Appendix D contributes the seed and nothing else.** Table 9 maps five economic conditions to rates (Boom 5%, Stable 8%, Moderate Inflation 10%, High Inflation 12%, Recession 15%) and introduces itself as "loan interest rates vary according to the prevailing economic conditions" — but the spec never defines how a condition is determined: there is no state for it, no rule that sets it, and the names do not map onto game state (only *Economic Recession* is an LK 18 event; *Economic Boom* is not — LK 18 has *Stock Market Boom* — and the two inflation conditions are not events at all). LK 13 and LK 14 by contrast give a concrete mechanism, naming loan interest rates among what inflation modifies and supplying the formula, so the rate drifts from the Stable Economy seed rather than being looked up. The other four rows are therefore unused. **Known consequence, accepted:** the drifted rate is not bounded by the table — across eight seeds it exceeds 15% in 79 round blocks and reaches 23% — which is also what makes **D29**'s saturation reachable. Clamping to Table 9's [5, 15] band was considered and declined; LK 14 states no ceiling |
...
| **D26** | Whether §5's vertical gaps are content | **Per block, not global — corrected.** The earlier reading emitted every block compact on the theory that the gaps were LaTeX spacing between verbatim chunks. Re-extracting §5 with blank lines made visible shows the document is deliberate and inconsistent: **six blocks carry real blank lines between their categories** — Paying Rent, Building Construction, Obtaining a Loan, Loan Repayment, Loan Default and Purchasing Insurance — while **Passing GO, Purchasing Property, Hotel Construction, Disaster, Auction, Economic Event, Government Regulation, Property Depreciation, Insurance Expiry, Bankruptcy and the Round Summary are genuinely compact.** The blocks invented here follow whichever §5 block they were shaped after: maintenance takes construction's blank, the LK 5 extend and increase actions take the loan block's, and **D31**'s redemption and **D32**'s sale take loan repayment's. The Round Summary keeps its 30 consecutive lines: the single blank the extraction shows inside it falls exactly at the page-22 break, which is the artifact the original reading correctly identified — it was only wrong to generalise from it. The `GAME OVER` block straddles the same kind of break and remains the one place §5 cannot arbitrate; it is still emitted compact |
